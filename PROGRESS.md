...
**Fase 6 — Integrações + PWA + RGPD (em curso).** Última sessão: **140** (2026-07-12, workflow do workbench: cards colapsados por fase + próxima acção + checklist + comissões).
...
- [ ] **Smoke sessão 140 (depois da mig + deploy):** abrir encomendas em fases diferentes → cards colapsados certos (com resumo, clique abre); banner "Próxima acção"; checklist da fase a marcar/persistir; estado → "Quadro enviado" com CTT → diálogo de tracking e link "Seguir" a abrir a página dos CTT com o objecto; página de um parceiro → bloco Comissões; Dashboard → alerta de comissões por pagar
...
> Sessões 127-133 movidas para o [PROGRESS-ARQUIVO.md](PROGRESS-ARQUIVO.md).
