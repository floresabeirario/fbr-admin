...
## Fase anterior: FASE 6 (parte 69) — Sessão 125: **Picker de templates — "Sugeridos para esta fase" mostrava ~18 templates com contradições** (2026-07-03). Screenshot da Maria (workbench da Melanie, pré-reserva): a lista de sugeridos tinha ~18 entradas, incluindo "tamanho escolhido" E "tamanho indeciso" ao mesmo tempo, condolências de funeral num casamento, "coberta por vale" sem vale, e cada template em PT+EN. **Causa:** `rankTemplatesForStatus` em [lib/templates.ts](src/lib/templates.ts) juntava as sugestões precisas por campos (fieldSuggestionBases) com TODAS as templates cujo `suggested_statuses` batia no estado actual (migs 041/080 marcam 9 bases × 2 idiomas para `entrega_flores_agendar`), sem filtro nenhum. **Fix (2 mecanismos, nada desaparece — o que não é sugerido desce para "Todos os templates"):** (1) mapa `RELEVANCIA_POR_CAMPOS` — uma template marcada para o estado só é sugerida se os campos não a contradisserem (funeral só se event_type=funeral; escolhido/indeciso conforme frame_size; vale só com gift_voucher_code; pedidos de sinal só com 100_por_pagar sem vale; CTT/recolha/mãos/dinheiro conforme o método de envio; orientacao_quadro só com tamanho por decidir); as condições espelham fieldSuggestionBases, por isso nunca excluem a template que essas regras escolheram; (2) `soIdiomaPreferido` — nos sugeridos, quando se sabe o idioma do cliente (form_language no workbench; "pt" no vale), a gémea no outro idioma desce para "Todos"; template sem gémea no idioma preferido mantém-se (melhor idioma errado que nada). Mesmo dedupe de idioma aplicado a `rankTemplatesForLead` (leads em /whatsapp tinham o mesmo PT+EN duplicado) [[feedback-aplicar-padroes-em-areas-analogas]]. A rota AI /api/whatsapp/suggest não precisou de mexida (já usava só fieldSuggestionBases). Resultado no caso da Melanie: ~18 sugeridas → 4 relevantes no idioma dela. **+5 testes** de rankTemplatesForStatus (cenário da Melanie, funeral, vale, template só-PT com cliente EN, sem orderFields = comportamento antigo) — 58 no total ✅. Sem migração. Preflight completo OK. **Maria — smoke:** workbench de uma pré-reserva → "Inserir template" deve mostrar 3-5 sugeridas coerentes com a encomenda e só no idioma do cliente; o resto está todo em "Todos os templates".

## Fase anterior: FASE 6 (parte 68) — Sessão 124: **Auditoria completa da plataforma + BACKUP diário para a Drive + hardening (mig 083) + CI** (2026-07-02/03). A Maria pediu análise profissional completa (inconsistências arquitectónicas, riscos de segurança, gaps, decisões questionáveis, sugestões); relatório entregue no chat e aprovado por item: **2a-d go, 3 go, 4 "avança com tudo", 8 "faz como achares melhor"**; 7.2 ok. **(1) BACKUP DIÁRIO — o gap mais grave (a spec prometia "backup automático diário para Google Drive" e não existia uma linha):** [lib/backup.ts](src/lib/backup.ts) + [lib/backup-rotation.ts](src/lib/backup-rotation.ts) (funções puras testadas; separadas porque backup.ts tem `server-only` que rebenta no vitest) + rota [/api/cron/backup](src/app/api/cron/backup/route.ts) (CRON_SECRET, maxDuration 60) + cron **05:00 UTC** no [vercel.json](vercel.json) (2 crons = limite do plano Hobby). Exporta **22 tabelas** (todas menos `google_integration` — contém o refresh_token OAuth, um segredo; se se perder reconecta-se em /settings/google em 30s) com paginação de 1000 (limite PostgREST) e ordenação estável (`id`; system_settings→`key`, team_members→`email`), JSON→gzip, para a pasta Drive **"Backups da base de dados"** sob "FBR — Encomendas" (id cacheado em system_settings[backup_drive_folder_id], revalidado se apagarem a pasta); ficheiro `fbr-backup-YYYY-MM-DD.json.gz`, re-corrida no mesmo dia substitui. **Rotação aprovada pela Maria** ("backups diários infinitos não servem para nada"): últimos 14 dias diários + dia 1 de cada mês no último ano + 1 de Janeiro para sempre (~26 ficheiros estáveis, 50-150 MB no total); os que saem vão para o **lixo da Drive** (recuperáveis 30 dias), nunca delete definitivo; ficheiros com nomes fora do padrão nunca são tocados. Estado de cada corrida (ok/erro, tamanho, nº registos, rodados) em system_settings[backup_status]; **healthcheck novo "Backup diário da BD → Drive"** em [healthchecks.ts](src/lib/healthchecks.ts): warning se nunca correu, ERROR se a última falhou ou tem >48h (backup partido em silêncio é pior que não ter backup). GRANTs service_role já cobriam as 22 tabelas (062+065+068+081) → **sem migração para o backup**. **13 testes novos** de rotação (53 no total ✅). NÃO testável localmente (.env.local só tem as chaves públicas) — teste real na produção (passos abaixo). **(2) Hardening aprovado:** (a) **gate de equipa** — `isTeamEmail()` novo em [roles.ts](src/lib/auth/roles.ts); [proxy.ts](src/proxy.ts) trata sessão de conta fora da equipa como NÃO autenticada (fica presa no /login, sem loop) e [getVerifiedEmail](src/lib/auth/server.ts) idem (cobre as rotas /api/whatsapp/* que o proxy deixa passar para o webhook); [auth/callback](src/app/auth/callback/route.ts) faz signOut a emails desconhecidos. Nota da auditoria: o login real é por **password** (não Google OAuth como a spec dizia) — o risco de signups abertos é via API directa do Supabase Auth; a Maria deve **desligar "Allow new users to sign up"** no dashboard (instruções no chat; com o gate, mesmo esquecido, um estranho já não entra). (b) [whatsapp/actions.ts](src/app/(admin)/whatsapp/actions.ts): `requireAdmin` nas 4 actions — era a ÚNICA família de actions sem verificação (as RLS só-admins seguravam; agora está alinhado com a convenção). (c) [/api/whatsapp/media/[fileId]](src/app/api/whatsapp/media/[fileId]/route.ts): antes servia QUALQUER ficheiro da Drive FBR a qualquer sessão (proxy de leitura arbitrário); agora só serve ficheiros presentes em `whatsapp_messages.media_drive_file_id`, lookup com a sessão do utilizador (RLS team_read decide) e refresh_token lido por service_role SÓ depois das verificações → **bónus: as imagens do WhatsApp deixam de aparecer partidas à Ana** (a RLS só-admins de google_integration dava-lhe 503; docstring do [admin.ts](src/lib/supabase/admin.ts) actualizada com o padrão). (d) **BUG REAL de produção encontrado e corrigido:** o upload de media do webhook WhatsApp→Drive **falhava SEMPRE à primeira** — `loadIntegration()` lia com a sessão (inexistente num webhook) → corria como `anon` → permission denied; a mig 065 deu o GRANT a service_role mas o código nunca o usava ali (as imagens só entravam na Drive pelo botão retry de um admin). Fix: [oauth.ts](src/lib/google/oauth.ts) e [drive.ts](src/lib/google/drive.ts) aceitam client Supabase opcional e o [media-fetch.ts](src/lib/whatsapp/media-fetch.ts) passa o seu createAdminClient. (e) **[Mig 083](supabase/migrations/083_crypto_random_ids.sql):** `generate_order_id()` e `generate_voucher_code()` passam de `random()` (não criptográfico; o order_id é o token público do site de status) para `gen_random_bytes()` do pgcrypto; o voucher mantém o alfabeto sem 0/O/I/1 e o loop de unicidade da mig 009 (32=2^5 chars → módulo sem viés). IDs existentes não mudam. (f) **xlsx do Google Search Console tirado de `public/`** (tudo o que está aí é servido no deploy!) para a pasta nova `_privado/`, gitignored — regra: nada entra em public/ que não seja para servir na app. **(3) CI:** [.github/workflows/ci.yml](.github/workflows/ci.yml) corre `npm run preflight` em cada push/PR com env vars dummy (validado localmente: o build passa com as mesmas dummies). Rede de segurança para esquecimentos e sessões paralelas. **(4) Nota de produto (pedido da Maria no chat):** a futura detecção de clientes duplicados **avisa com link mas NUNCA bloqueia** — a mesma pessoa pode fazer várias encomendas (memória nova feedback_duplicados_nao_bloquear.md). Preflight completo OK (tsc + 53 testes + build). **Maria — passos manuais:** (1) correr a **[mig 083](supabase/migrations/083_crypto_random_ids.sql)** no Supabase SQL Editor [[feedback-migracoes-supabase-aplicadas]]; (2) **desligar signups** no Supabase: menu esquerdo → Authentication → Sign In / Up → "Allow new users to sign up" OFF → Save; (3) push (estreia o CI — aba Actions no GitHub deve ficar verde); (4) smoke: **login dos 3 perfis** (a mudança do proxy é a mais sensível!), conversa WhatsApp com imagens (pedir à Ana para confirmar que agora as vê), Sistema→Healthchecks com a linha nova "Backup diário da BD → Drive" (amarela até ao 1º backup); (5) **testar o backup já**: Vercel → projecto → Settings → Crons → /api/cron/backup → Run; confirmar na Drive a pasta "Backups da base de dados" com `fbr-backup-<data>.json.gz` e o healthcheck verde no dia seguinte. **Roadmap aprovado para as próximas sessões (por ordem):** (i) 2b — form público validado no servidor (Turnstile server-side no repo fbr-website + revogar INSERT anon; migração); (ii) 3 — centralizar equipa (adoptar is_team_admin()/team_members da mig 046 nas ~24 policies + código TS); (iii) 2c — expurgar conversas WhatsApp do histórico git (sessão DEDICADA: filter-repo + force push, coordenar com sessões paralelas); (iv) 4 restantes — tipos gerados do Supabase no preflight, monitorização de erros, refactor do workbench de Preservação; (v) 8 — funcionalidades ao critério do Claude: vista "Hoje" no Dashboard, notificações push internas (PWA), duplicados (não bloqueantes!), relatório mensal interno; (vi) retomar a cadência de comunicação (sessão 104). **PARTE 2 (mesma sessão, "continua a fazer as coisas"; a Maria correu a mig 083 e desligou os signups):** **(5) Item 2b FEITO — forms do site fechados de ponta a ponta:** a auditoria ao fbr-website revelou que as rotas [api/reservar-preservacao](../fbr-website/fbr-website/app/api/reservar-preservacao/route.js) e [api/vale-presente](../fbr-website/fbr-website/app/api/vale-presente/route.js) JÁ validavam Turnstile server-side + honeypot + rate limit + Origin — o único buraco era escreverem com a ANON KEY (pública), o que obrigava a policy de INSERT anon a existir (= tudo contornável falando directo com o PostgREST). Fix: helper novo [app/_lib/supabase-server.js](../fbr-website/fbr-website/app/_lib/supabase-server.js) (`createFormsClient` prefere **SUPABASE_SERVICE_ROLE_KEY**, com fallback anon durante a transição, + `createAnonClient`); as 2 rotas + o teste de ESCRITA do [api/health](../fbr-website/fbr-website/app/api/health/route.js) usam o forms client (a LEITURA do health continua anon — testa o que os sites públicos de status/voucher veem). Build do site OK; **commit 56d162b no develop do fbr-website, PUSHED** (merge→main decide a Maria). No admin, **[mig 084](supabase/migrations/084_revoke_anon_inserts.sql)**: drop das policies orders/vouchers_public_insert (016) e *_public_select_recent (017, só serviam o RETURNING dos inserts anon), REVOKE INSERT em orders/vouchers/audit_log ao anon, REVOKE EXECUTE nas funções de gerar IDs e update_updated_at, GRANT EXECUTE de cleanup_form_healthchecks a service_role (+revoke ao anon). Policies de leitura dos sites públicos INTACTAS. **⚠️ ORDEM CRÍTICA:** (1) env var `SUPABASE_SERVICE_ROLE_KEY` no projecto **fbr-website** da Vercel; (2) merge develop→main do site + esperar o deploy; (3) SÓ DEPOIS correr a mig 084; (4) verificar floresabeirario.pt/api/health → ok:true + submeter um form de teste. Se a mig correr antes do deploy, os forms PARTEM. **(6) Sobreposições no workbench mobile (screenshot da Maria: "Tamanho da moldura"/"Fundo do quadro" e "Moldura pirâmide"/"Tipo de moldura" texto em cima de texto):** causa raiz = `Field`/`HeroField` em [_components/layout.tsx](src/app/(admin)/preservacao/[id]/_components/layout.tsx) usavam `col-span-2` FIXO; num grid `grid-cols-1` (mobile) um span de 2 cria uma 2ª coluna IMPLÍCITA e os campos auto-colocados ficam em 2 colunas esmagadas com overflow sobreposto. Fix: `span2` → `xl:col-span-2` no Field (o Grid2 só tem 2 colunas em xl) e `lg:col-span-2` no HeroField (grid do hero é lg); desktop intocado [[feedback-desktop-prioridade]]. **Varrimento completo do repo:** todos os restantes `col-span-N` têm prefixo responsivo alinhado com o grid pai ou vivem em grids de largura fixa (grid-cols-3/12) — este padrão de sobreposição não existe em mais lado nenhum. Preflight completo OK (tsc + 53 testes + build) depois da parte 2. **(7) 2b FECHADO EM PRODUÇÃO na própria sessão:** Maria adicionou a env var na Vercel do site, o Claude fez o merge develop→main (**88ca763**, deploy READY verificado via Vercel MCP), e a Maria correu a **mig 084**. Verificação completa pós-migração: /api/health do site **ok:true** (o teste de escrita usa o caminho novo; confirmado nos logs da Vercel que foi servido pelo deployment novo e SEM o aviso de fallback anon), home + 2 páginas de forms **200**, leituras anon de orders/vouchers (sites status/voucher) **200**, sites status.floresabeirario.pt e voucher.floresabeirario.pt **200**, e **INSERT anon directo ao PostgREST devolve 42501 permission denied** — o contorno do Turnstile está morto. Migs 083 e 084 corridas; signups do Supabase Auth desligados pela Maria (confirmar Save changes). **Pendente do admin:** push do fbr-admin (Maria) + teste do cron /api/cron/backup na Vercel + smoke do login dos 3 perfis. **PARTE 3 ("push admin" + "continua próxima tarefa"):** push feito pelo Claude — commits **c1b6931** (sessão 124) + **cbfc19e** (trabalho da sessão paralela 125 em templates.ts, committado em separado com 58 testes verdes + tsc limpo) + o 6210a3f da 122 que estava por pushar. **Item 3 do roadmap FEITO na BD: [mig 085](supabase/migrations/085_team_policies_centralized.sql)** — TODAS as policies com os 3 emails hardcoded passam a delegar em `is_team_admin()`/`is_team_member()` (a mig 046 finalmente adoptada): orders (re-aplica), vouchers, public_status_settings, audit_log (INSERT apertado de `true`→is_team_member), tasks, personal_checklist (owner_all intacta), partners, competitors, google_integration, pricing_items, ideas, recipes, public_figures, chat_messages (lógica de autor mantida), expenses, production_cost_items, system_settings (+select true→member), message_templates, team_members (funções SECURITY DEFINER, sem recursão), task_templates, whatsapp_conversations/messages, claude_usage — **~35 policies em 22 tabelas**, policies anon intactas. A mig é AUTO-CONTIDA (re-cria funções+seed, idempotente, funciona quer a 046 tenha corrido em produção quer não [[feedback-migracoes-supabase-aplicadas]]) e termina com query de verificação (0 policies com emails hardcoded). A partir daqui, mudar papéis/adicionar membro = 1 linha em team_members. **Falta a parte TS** (roles.ts/team-members.ts/login/chat hardcoded — próxima sessão, com UI de gestão em Sistema). **Maria:** correr a **mig 085** no SQL Editor (+ verificações do fim do ficheiro); smoke: login dos 3, Ana continua só-leitura fora de Tarefas/Parcerias. **PARTE 4 (mig 085 corrida pela Maria + "continua"):** verificação pós-085 feita (leituras anon dos sites públicos 200 + health do site ok:true; a parte authenticated fica no smoke do login dela). **Item 3 — parte TS FEITA:** [roles.ts](src/lib/auth/roles.ts) ganha `TEAM: TeamMember[]` (email+nome+foto+papel) como **fonte única no código** — ADMIN_EMAILS_LIST/VIEWER_EMAILS_LIST agora derivam de TEAM; os 5 consumidores que duplicavam a lista passaram a importar/derivar: [team-members.ts](src/app/(admin)/_components/dashboard/team-members.ts) (re-exporta), [layout.tsx](src/app/(admin)/layout.tsx) (PROFILES), [login/page.tsx](src/app/login/page.tsx) (PROFILES), [chat-client.tsx](src/app/(admin)/chat/chat-client.tsx) (TEAM + mapa local CHAT_COLORS por email, cosmético), workbenches de [parcerias](src/app/(admin)/parcerias/[id]/workbench-client.tsx) e [figuras](src/app/(admin)/parcerias/figura/[id]/workbench-client.tsx) (ASSIGNEES). Grep confirma: **zero emails da equipa hardcoded fora de roles.ts** (+ mapa de cores do chat). Porquê estático e não a ler da BD: o /login mostra os perfis ANTES de haver sessão e o anon não pode ler team_members — documentado no próprio roles.ts. **Adicionar/mudar um membro agora = 1 linha em roles.ts + 1 linha em team_members (BD) + password no Supabase Auth.** Preflight OK (tsc + 58 testes + build) + lint limpo nos 7 ficheiros.

## Fase anterior: FASE 6 (parte 67) — Sessão 123: **fbr-tracking — timeline com as 12 fases (BUG: passo errado a partir da fase 7) + página no idioma do cliente + polish** (2026-07-02). A Maria pediu review de design/estrutura do site status.floresabeirario.pt (repo fbr-tracking) e aprovou avançar ("go"), com a instrução de NÃO ir para main → **branch `develop` novo, commits 07116d3+1137a39+2343062, PUSHED** (a Vercel gera preview do develop; merge para main só com OK dela). **(1) BUG encontrado e corrigido:** a timeline vertical em [pages/[id].js](../fbr-tracking/fbr-tracking/fbr-tracking/pages/[id].js) tinha **11 passos mas o admin tem 12 fases públicas** — faltava a fase 7 "A finalizar o quadro" (nasceu no admin na sessão 91 e o `PASSOS` do tracking nunca foi actualizado). Da fase 7 em diante o cliente via o passo ERRADO destacado (ex.: encomenda "a ser emoldurado" destacava "A ser fotografado") e uma encomenda concluída via "Passo 12 de 11" com a barra a 110%. **Fix estrutural:** passos movidos para [utils/timeline.js](../fbr-tracking/fbr-tracking/fbr-tracking/utils/timeline.js) novo (12 entradas, comentário a exigir 1 entrada por fase e sincronia com STATUS_TO_PUBLIC_PHASE; ficheiro próprio sem import do Supabase porque é usado no render do browser — importar utils/supabase.js no cliente rebentava com env vars em falta). Encomenda concluída (fase 12): último passo fica ✓ em vez de "Em curso". **(2) Página no idioma do cliente:** `getEncomendaById` em [utils/supabase.js](../fbr-tracking/fbr-tracking/fbr-tracking/utils/supabase.js) passa a devolver `idioma` ('pt'|'en'|'ambos' — o que a Maria escolhe na aba Status); os textos fixos da página (botões "Fale connosco"/"Visitar Site", "Estado Atual", "Atualizado a", "Progresso/Passo X de 12", "Entrega estimada", intro) deixam de estar SEMPRE bilingues e respeitam o idioma (helper `bi(pt,en)`; 'ambos' mantém "PT / EN"; timeline em 'ambos' mostra EN em itálico pequeno sob cada passo; mensagem do WhatsApp já era localizada). Fallback por fase/fase_en para links antigos. **(3) Polish:** secção "Entrega estimada" sem data mostra **"Em breve"/"Coming soon"** no idioma do cliente (ronda 2, pedido da Maria — a v1 escondia a secção; canceladas não a mostram de todo, `getEncomendaById` devolve boolean `cancelada`); **og:image + og:url** no preview do link (WhatsApp/iMessage mostram o ícone); toggle do progresso passou de `<div role=button>` a `<button>` real (teclado + aria-expanded); hover/focus states via classes `.fbr-btn/.fbr-input/.fbr-social/.fbr-toggle` em [_document.js](../fbr-tracking/fbr-tracking/fbr-tracking/pages/_document.js) (só propriedades que o inline não define — filter/transform/box-shadow — para não perder para os estilos inline); viewport sem `maximum-scale=1` (zoom permitido, acessibilidade); placeholder da pesquisa "Ex: 2024001" (formato antigo enganador) → "Código da encomenda / Order code" + autoComplete off; `font-display:swap` na TanMemories. **(4) Verificação:** build OK (env dummy) + smoke `next start`: home e página de erro renderizam; a página de encomenda REAL não é testável localmente (sem credenciais). **NÃO mexido (decisões para a Maria):** timeline continua colapsada por defeito; identidade visual verde mantida; /api/tracking mantido (endpoint público que ninguém no admin usa — remover?); sugestões maiores discutidas no chat (foto da peça na fase pronta, redesign). **RONDA 4 (aprovação da Maria "podes passar para o main… quero que o site fique brutal", 99% mobile):** (a) **merge develop→main 2343062 pushed** — produção verificada por curl (placeholder novo no ar em status.floresabeirario.pt); (b) **redesign mobile-first no develop (commit 011d954, pushed):** estilos inline → [styles/globals.css](../fbr-tracking/fbr-tracking/fbr-tracking/styles/globals.css) (via `_app.js` novo) + cabeçalho/rodapé partilhados em [components/chrome.js](../fbr-tracking/fbr-tracking/fbr-tracking/components/chrome.js) (eram 3 cópias); telemóvel = cartão full-screen tipo app (rodapé no fundo, `theme-color` verde na barra do browser, apple-touch-icon, input 16px anti-zoom iOS); animações discretas (secções em cascata, barra de progresso cresce ao carregar, ponto da fase actual pulsa, timeline expande com transição grid-template-rows, `prefers-reduced-motion` respeitado); barra de progresso larga com gradiente (era 56px); hero com gradiente + raminhos botânicos SVG desenhados à mão (opacidade ~12%); tipografia clamp(); **encomenda concluída troca "Entrega estimada" por botão "⭐ Deixar avaliação no Google"** (o link já vivia na mensagem default da fase 12); cancelada = fase a cinzento neutro. Build + smoke local OK (home, erro). **RONDA 5:** Maria aprovou o redesign ("gosto de tudo") excepto o emoji ✿ → removido dos botões "Visitar Site" (3 páginas) e do ornamento da entrega estimada (classe .orn apagada); **merge develop→main f225fdf PUSHED e produção VERIFICADA por curl** (redesign no ar, página de erro OK, /api/tracking devolve 404 como esperado). develop e main estão iguais (f225fdf). **RONDA 6 ("não queres fazer um redesign?"):** redesign conceptual **"Herbário" SÓ NO DEVELOP (commit 7b2f46c, pushed, main INTACTO até aprovação)** — fundo passa de cartão-branco-sobre-cinzento para **papel cream de arquivo** (#F6F1E7, alinha com o site da marca; theme-color acompanha); banner escuro do topo → wordmark discreto TanMemories; o verde-escuro concentra-se num **"palco" do estado actual**: anel de progresso SVG animado (X/12, ✓ quando concluída), fase em TanMemories itálico, 12 pontinhos com o actual a pulsar, raminho em marca de água; timeline expansível sob link "Ver as 12 etapas"; mensagem em cartão paper-white; **entrega estimada dentro de uma moldura desenhada com traço duplo** (eco do quadro); cancelada = palco em tom neutro. Toda a lógica (idioma, Em breve, review CTA, reduced-motion) intacta. Build + smoke OK. **RONDA 7 (Maria: "achas mesmo que o novo está melhor?" → análise honesta no chat: palco ganha, fluxo perde; Maria: "faz melhorias no novo. gosto de cores tipo primavera") → Herbário v3 (commit 609ef9b, develop, pushed):** mensagem da equipa COLADA ao palco (o "Ver as 12 etapas" desce para depois dela — fase+texto voltam a ler-se como história); "Passo X de 12" em texto removido do palco (anel já o diz; sr-only para leitores de ecrã); nome da cliente encolhido para o tamanho do design de produção; **paleta primavera**: verdes frescos (g600 #4A7A57), fundo marfim esverdeado #F5F7EE, linhas verde-claras, moldura verde-dourada, apontamentos **rosa-flor** (traço sob o wordmark + bolinha da fase actual a pulsar) e amarelo-manteiga no botão de avaliação. Build + smoke OK. **RONDA 8 (Maria: "a fase e a mensagem têm que estar juntas… temos que tentar outra coisa… sê mais criativo, chuva de flores?") → Herbário v4 (commit b10d05c, develop, pushed):** layout da info repensado — **UM só cartão de estado com duas metades coladas** (topo verde-escuro: anel+fase+bolinhas; base papel claro: mensagem+atualizado), o cartão .note separado morreu; **chuva de pétalas** 🌸 nova em [chrome.js](../fbr-tracking/fbr-tracking/fbr-tracking/components/chrome.js) (`Petals`): 11 pétalas CSS puras (rosa/lilás/manteiga/verde-folha) a cair com balanço atrás do conteúdo nas 3 páginas, valores determinísticos (sem Math.random → sem hydration mismatch), delays negativos (ecrã nasce polvilhado), prefers-reduced-motion pára tudo. Build + smoke OK. **RONDA 9 (Maria: "não gosto das pétalas… card esquisito… tens 12 etapas 2 vezes, no original só se via 1… está muito verde") → Herbário v5 (commit edd5cc8, develop, pushed):** pétalas removidas (componente Petals apagado); cartão do estado = **UM cartão claro** (anel + Estado atual + fase + mensagem + atualizado juntos, como o original que lia melhor); **12 bolinhas mortas** — etapas aparecem UMA vez (anel = resumo X/12; lista expansível = detalhe); **primavera nas cores** (página estava verde demais): fio pastel rosa/lilás/manteiga/verde-folha no topo do cartão, anel em degradê verde→rosa, véu blush no topo da página + theme-color #FAF0EC, eyebrow rosa-flor, raminho rosa atrás do cabeçalho (`page-flor`), pill lilás, passo actual pulsa rosa, foco do input rosa, círculos do rodapé pastel, neutros aquecidos. Build + smoke OK. **RONDA 10 (Maria: gradiente da barra horizontal fora; "6/12 quê? batatas?"; o progresso tem de ser o próprio botão do dropdown; "várias flores diferentes" em vez do "pau com um sol") → Herbário v6 (commit f2823e7, develop, pushed):** barra degradê pastel e anel removidos; **linha de progresso no rodapé do cartão do estado ("A viagem · Etapa 6 de 12" + barra verde) é o próprio botão que abre as 12 etapas dentro do cartão**; raminho decorativo novo `BouquetSvg` com 4 flores diferentes desenhadas à mão (margarida rosa, tulipa lilás, flor 5 pétalas manteiga, folhas verdes), substitui o FlorSvg. **RONDA 11 (Maria: wordmark e nome do cliente "basicamente iguais"; flores era "espalhadas pelo fundo, tipo papel de parede suave"; "A viagem" desnecessário) → Herbário v7 (commit c300935, develop, pushed):** **flores em padrão de fundo** — tile SVG 280px inline no CSS de [globals.css](../fbr-tracking/fbr-tracking/fbr-tracking/styles/globals.css) (6 motivos pastel a ~30% opacidade, repetido pela página, véu blush por cima; BouquetSvg do canto removido); **wordmark 30→21px verde médio** (nome da cliente é o protagonista, 28-33px); rótulo "A viagem" removido (fica "Etapa 6 de 12" + barra + chevron). **RONDA 12 (Maria: "o ovo e o raminho só com uma folha são esquisitos; gosto do papel de parede; vê as cores do texto, não ficaram coerentes") → Herbário v8 (commit 8f27857, develop, pushed):** no papel de parede, tulipa-ovo → **alfazema lilás**, folhinha ganhou 2ª folha, botões de rosa ganharam sépalas+miolo; **coerência de cores** (sistema documentado no topo do globals.css): TanMemories grande TODO em g800 (a fase estava em g600), rótulos pequenos uppercase todos rosa-flor (eyebrow + ESTADO ATUAL combinam), msg EN e fundos progresso/etapas sem resto esverdeado. Build + smoke OK. **RONDA 13 (Maria mandou screenshots develop vs main: "não gosto muito do develop… não sei porquê" → diagnóstico no chat: 3 acentos que não conversam (rosa+lilás+manteiga), papel de parede pastel a ler "cartão de aniversário" vs o posicionamento premium, e o cabeçalho sem peso; a estrutura do Herbário é que estava melhor. Maria: "faz no develop como achares melhor") → Herbário v9 (commit bc23554, develop, PUSHED):** **estrutura do Herbário na paleta do main** — voltam o cabeçalho verde-escuro com FlorSvg line-art, o cartão branco full-screen no telemóvel e a paleta 100% verdes FBR do main (globals.css reescrito; :root do main); mantém-se do Herbário: nome do cliente primeiro (eyebrow "A viagem das suas flores" agora em g600), cartão do estado com fase+mensagem juntas e a linha de progresso clicável no rodapé a abrir as 12 etapas, entrega estimada na moldura de traço duplo (recolorida g200 sobre branco) e botões de ação; pill "Atualizado a" branca neutra (morreu a lilás), theme-color de volta a #2F3E32, chrome.js = versão do main (Hero+Footer neutro; Mast e círculos pastel do rodapé mortos). Build + smoke `next start` OK (home e página de erro com a estrutura nova). Main continua com design da ronda 5; Maria vê o preview do develop na Vercel e decide o merge. **RONDA 14 (Maria: "gosto. em vez de dois paus com folhas, não podes pôr flores?") → v9.1 (commit 0503cf7, develop, PUSHED):** os dois FlorSvg do cabeçalho (caule+2 folhas+florzinha) deram lugar a flores a sério em line-art no [chrome.js](../fbr-tracking/fbr-tracking/fbr-tracking/components/chrome.js): **margarida de 8 pétalas** (elipses rodadas) com folha no canto superior direito e **flor de campo de 5 pétalas** com 2 folhas no canto inferior esquerdo; a rotação do canto de baixo passou de -158° (desenho ao contrário, só se via o caule) para -12° com bottom -52px (flor visível, caule cortado pela margem). Build + smoke OK. **RONDA 15 (Maria: "parece um pouco infantil, não sei como explicar") → v9.2 (commit 25b9e85, develop, PUSHED):** o infantil era a simetria perfeita das pétalas-elipse em roda; flores redesenhadas em **estilo ilustração botânica, vistas de lado**: flor aberta com 5 pétalas em leque de alturas irregulares + botão em haste secundária (canto superior direito) e flor de 3 pétalas com 2 folhas (canto inferior esquerdo), tudo pétalas em teardrop desenhadas com beziers, sem simetria radial. Build OK. Estilo guardado em memória (feedback_flores_lineart_estilo.md). **Maria aprovou ("gosto muito. mete no main") → merge develop→main FAST-FORWARD para 25b9e85, PUSHED, produção VERIFICADA por curl** (status.floresabeirario.pt já serve as flores novas + cartão/hero). develop e main estão iguais (25b9e85). Sessão 123 fechada: o design final é a estrutura Herbário na paleta verde do main com flores botânicas no cabeçalho. **Dúvidas da Maria respondidas no chat:** og:image = o MESMO icon.png do favicon que ela já gosta (só torna o preview mais fiável, nada muda visualmente); /api/tracking explicado (endpoint público não usado, herança da era Google Sheets) → **ronda 3 (screenshot do preview):** Maria mandou removê-lo (`pages/api/tracking.js` apagado) + slogan do cabeçalho ("Especialistas em preservação de flores · Flower preservation specialists") estava sempre bilingue → passa a seguir o idioma na página da encomenda (home/erro continuam bilingues).

## Fase anterior: FASE 6 (parte 66) — Sessão 122: **Site — auditoria UX/UI/copy/SEO completa + correções aprovadas (32 ficheiros, build+smoke OK)** (2026-07-02). A Maria pediu análise crítica de todo o site como especialista UX/UI/copy/SEO; relatório entregue no chat e depois aprovou correcções por item ("faz todos os altos… todos os médios excepto…"). **Tudo no repo fbr-website, branch develop, SEM commit (Maria decide).** **(1) ALTOS globais:** (a) **funil EN reparado** — TODOS os CTAs que mandavam utilizadores EN para páginas PT passam a locale-aware (`bookHref` etc.): HomeHero, HomeSteps (3 links), HomeClient (sustentabilidade/oferecer/contactos/CTA noivas), Opções CTA final, FAQ (CTA + mensagem WhatsApp que estava fixa em PT → variante nova `duvida` em [wa.js](../fbr-website/fbr-website/app/_lib/wa.js) PT+EN), Sustentabilidade CTA, hint de preços do form do vale; (b) **blog EN 404** — cards de /en/blog apontavam para `/blog/<slug-en>` sem prefixo (404 para visitantes sem cookie de locale); [BlogClient.jsx](../fbr-website/fbr-website/app/blog/BlogClient.jsx) ganha `blogBase` com `/en`; (c) **navegação `<a>` → `Link`** (client-side, sem full page reload): Nav (tudo, incl. logos via `motion.create(Link)`), Footer, Home completa, blog, CTAs das páginas tocadas — o site "sente-se" muito mais rápido; (d) **LangSwitcher do Footer** partia em artigos do blog (template `/blog/[slug]` sem params) → replica a lógica da Nav (`useAltLocaleHref`+params). **(2) MÉDIOS (excepto contraste/tipografia — sessão futura — e Elfsight/cookies — Maria decide):** dropdowns desktop (serviços+idioma) acessíveis por teclado (onFocus/onBlur, aria-haspopup, toggle no clique); `<main>` aninhado → `<div>` em 5 páginas (momentos-especiais, enviar-flores, vale-presente, privacidade, termos — o layout já tem `<main>`); **404/erro com marca**: [app/[locale]/not-found.js](../fbr-website/fbr-website/app/[locale]/not-found.js) + `[...rest]/page.js` (catch-all→notFound) + [error.js](../fbr-website/fbr-website/app/[locale]/error.js) + [app/not-found.js](../fbr-website/fbr-website/app/not-found.js) raiz (bilingue, para 1º segmento inválido); aria-labels/alts que estavam hardcoded em PT nas páginas EN passam a t() (HomeClient secções+vídeo, foto APCC com keys novas `apccImgAlt`, slider do vale `valeAlt`, equipa em Contactos, Sustentabilidade usa keys existentes — os "tuas"/sem acentos morreram); registo **"tu"→"você" no blog** (meta description, eyebrow "Do atelier para si", heroDesc); **copy EN de videochamada removida** (equipa não faz calls em EN [[project-chamadas-idioma]]): contactos meta/og/keywords, faq.apoioDesc, home.maisAjudaDesc1 → WhatsApp/email; sitemap: `lastModified: new Date()` em tudo → só artigos do blog com a data real do frontmatter. **(3) Por página:** Home — **h1 passou a ser a eyebrow "Especialistas em preservação de flores"** (mesmo aspecto visual; nome da marca vira `<p>` idêntico) para o Google ler o tema; vídeo do tracking ganha `<source>` MP4 fallback (**⚠️ falta a Maria exportar `public/videos/tracking.mp4`** — sem ele, iPhones antigos continuam sem vídeo) e aria via t(). Como Funciona — HowTo schema **localizado PT/EN** + URLs com www (era apex) + preço via precos.js; foto do passo 1 já não abre em novo separador. Oferecer — **CTA "Ver preços" apontava para o formulário de reserva** → /opcoes-e-precos. Preços **centralizados em [_lib/precos.js](../fbr-website/fbr-website/app/_lib/precos.js)** (Opções 300/400/500/90/35/35 + Emoldurar 200/270/360; ⚠️ meta title de Opções tem "300€-500€" à mão nas messages). **Splits de título frágeis** (`t("h1").split(t("em"))[0]` — se as traduções dessincronizassem, o texto duplicava) → helper [_lib/splitTitle.js](../fbr-website/fbr-website/app/_lib/splitTitle.js) com guarda, aplicado em 9 ficheiros (4 momentos ×3 títulos via codemod, momentos-especiais, preservação, como-funciona, enviar-flores, bouquet ×2). **Form reserva** — botão deixou de ficar disabled mudo quando o Turnstile não carrega; mensagem `erroTurnstile` PT+EN. **Form vale** — **checkbox Termos/RGPD novo** (obrigatório, keys `termosLabel/termosLink/erroTermos`, link localizado; CSS vf-check/vf-termos novo) + **resumo de erros clicável** igual ao da reserva (FIELD_LABEL_KEYS+data-field+focusField) + erroTurnstile. Recriação — fragment com key no map + subject do mailto localizado. **(4) Verificação:** build OK; smoke com `next start` — 17 URLs 200/404 correctos, cards EN do blog com `/en/blog/...`, 404 raiz e de locale com marca, CTAs EN certos, schema EN no how-it-works, termos no form do vale. **Perguntas da Maria respondidas no chat (SEM mexer):** aggregateRating (confirmei: guidelines Google proíbem self-serving desde 2019 — decisão dela), keywords meta (confirmei: ignorada desde 2009 — deixei estar), baptizado/batizado (mantêm-se os dois de propósito por SEO), OG image única (mantém), Elfsight (fica; cookies explicado no chat), ordem de Opções e Preços (estratégia dela de aliciar primeiro — mantém, sugerido âncora), UltraVue width/height (explicado, não mexido). **PARTE 2 (aprovação da Maria):** botão do hero da home renomeado "Reservar Data" → **"Reservar eternização de flores"** (key nova `home.heroCTALong` PT + EN "Book flower preservation"; o "Reservar Data" mantém-se na nav e no CTA das noivas); build + smoke OK (hero PT/EN certos, nav intacta). **Git: TUDO EM PRODUÇÃO** — develop **2baaf4d** + merge develop→main **af2540a**, ambos pushed (Vercel faz deploy automático do main). **Pendentes:** (1) Maria exporta o vídeo do tracking em MP4 (H.264) para `public/videos/tracking.mp4` (o código já tem o fallback; sem o ficheiro, iPhones antigos veem caixa escura); (2) Maria vai pensar nas perguntas em aberto e diz depois o que fazer: remover aggregateRating do schema? remover keywords meta? remover VideoObject da home? subtítulo no hero? width/height na imagem UltraVue? data "última atualização" nas páginas legais? banner de cookies para o Elfsight? (3) smoke pós-deploy: home → botão "Reservar eternização de flores"; home EN → "Book flower preservation" leva ao form EN; /en/blog → abrir um artigo; trocar idioma no footer dentro de um artigo; URL inventado → 404 com marca; form do vale → termos obrigatórios.

## Fase anterior: FASE 6 (parte 65) — Sessão 121: **Performance (getClaims + loading global) + "Entregar até" (mig 082) + refresh visível no PC + fixes tabela + tabelas mobile** (2026-07-02). Pedidos da Maria: app "demora imenso a carregar", melhorias mobile, registar prazos de entrega pedidos pelo cliente ("até ao aniversário X"), botão de refresh "não existe" no PC; + screenshot com "100% por pagar" cortado na tabela. **(1) Performance — 2 causas atacadas:** (a) o [proxy.ts](src/proxy.ts) fazia `auth.getUser()` = chamada de REDE ao Supabase Auth em CADA pedido, e cada página repetia via `getCurrentEmail()`/`getCurrentRole()` → **≥2 idas ao Auth antes de tocar nos dados**. Ambos passam a **`auth.getClaims()`** ([lib/auth/server.ts](src/lib/auth/server.ts), helper novo `getVerifiedEmail`): valida a assinatura do JWT **localmente** (chaves públicas cacheadas) e continua a refrescar sessões a expirar; se o projecto ainda usar chave simétrica legacy faz fallback ao servidor (= custo de hoje, nunca pior). `requireAdmin`/`requireUser` continuam criptograficamente seguros (getClaims verifica a assinatura — não é ler o cookie às cegas). (b) **[loading.tsx](src/app/(admin)/loading.tsx) novo ao nível do grupo (admin)** — as 13 páginas server-side ganham spinner imediato (sidebar visível) em vez de ecrã parado até a query acabar; os loading próprios dos workbenches mantêm-se. Nota para a Maria: o resto da lentidão no ARRANQUE da PWA é cold start da Vercel (função a "acordar"; melhorável com Fluid Compute nas settings da Vercel, não no código). **(2) "Entregar até" — [mig 082](supabase/migrations/082_delivery_deadline.sql):** `orders.delivery_deadline DATE` + `delivery_deadline_reason TEXT` (só ALTER, sem grants novos; anon não vê). Workbench (caixa "Envio das flores e receção do quadro", secção Receção do quadro): campos **"Entregar até" + "Motivo do prazo"** + aviso âmbar (≤30 dias) / rose (≤14 dias ou já passou), escondido quando enviado/recebido/cancelado. Tabela de Preservação: **pill "⏰ até dd/MM"** ao lado do nome (âmbar; rose ≤14 dias; tooltip com data completa+motivo). [dashboard.ts](src/lib/dashboard.ts): **alerta 1c** "⏰ Entregar até dd/mm (X dias)" (warn ≤30d; danger ≤14d ou passado; esconde enviado/recebido/cancelado). Export CSV ([export-csv.ts](src/lib/export-csv.ts)) + RGPD-print incluem os 2 campos. **(3) Refresh no PC:** o botão JÁ existia mas escondido no rodapé da sidebar (Maria nunca o viu, fazia F5) — **duplicado no topo da sidebar, ao lado do logo "FBR Admin"** ([layout.tsx](src/app/(admin)/layout.tsx); desktop expandido; o do rodapé e o do header mobile mantêm-se). **(4) Tabela de Preservação no PC (screenshot da Maria):** coluna Pagamento 150→**170px** ("100% por pagar"+chevron ficava cortado) e Estado 200→**235px** (o trigger tem `max-w-[220px]` e transbordava da coluna por cima do Orçamento); soma do `minWidth` 830→**885** ([preservacao-client.tsx](src/app/(admin)/preservacao/preservacao-client.tsx)). **(5) Mobile — tabelas com scroll horizontal em vez de esmagar** (`overflow-x-auto` no wrapper + `min-w-[…]` na tabela; desktop intocado [[feedback-desktop-prioridade]]): Finanças→Catálogo ×2 (720/560px), Finanças→Custos ×2 (impressão de foto 420px, consumíveis 900px), Finanças→Painel P&L por categoria (640px), Métricas→Top 5 parceiros (560px), Sistema→Templates de tarefas (680px). A tabela de Preservação e as de Parcerias/Status/Vale já tinham. Preflight completo OK (tsc + 40 testes + build). **Maria — passos manuais:** (1) **correr a [mig 082](supabase/migrations/082_delivery_deadline.sql) no Supabase SQL Editor ANTES do push** [[feedback-migracoes-supabase-aplicadas]] — sem ela, gravar "Entregar até" no workbench dá erro (coluna inexistente); (2) push para Vercel; (3) smoke: **login continua a funcionar** (mudança do getClaims é a mais sensível!), navegação entre abas mostra spinner e nota-se mais rápida, workbench → campos "Entregar até"/"Motivo" na caixa de envio + pill ⏰ na tabela + alerta no Dashboard, "100% por pagar" inteiro na tabela, refresh ⟳ ao lado do logo, Finanças/Métricas no telemóvel com scroll horizontal nas tabelas largas. **Mobile — nota:** a Maria disse "há vários problemas" no telemóvel sem especificar; esta sessão atacou o padrão mais óbvio (tabelas esmagadas). Se ela mandar screenshots dos sítios concretos que estão mal, corrigem-se cirurgicamente na próxima.
