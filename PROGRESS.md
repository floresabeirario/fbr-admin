...
## Fase anterior: FASE 6 (parte 57) — Sessão 113: **Auditoria das Métricas + botão refresh global** (2026-06-09). A Maria notou que as estatísticas das Métricas "não pareciam bem" e pediu auditoria exímia + um botão de refresh geral e discreto (instalou a app no PC como PWA). **Auditoria — encontrados 4 problemas em [metrics.ts](src/lib/metrics.ts):** (1) **encomendas canceladas contavam para a receita** (orderRevenue/totalRevenue/monthlyRevenue/topPartners nunca filtravam `status === "cancelado"`); (2) **receita contava pela data de criação** (`created_at`) enquanto as Finanças contam pela **data do evento** (`event_date`) → os dois números nunca batiam, apesar do comentário no topo dizer que "tem de bater certo com financas-client.tsx"; (3) **comissões do Top 5 parceiros sobrestimadas** — somavam `partner_commission` em bruto, incluindo estados `N/A`/`Não aceita` e sem proporção do %pago; (4) etiqueta "vs. mês anterior" fixa no cartão "Encomendas novas" mesmo em presets anuais/trimestrais. **Correções (a Maria aprovou "só os cálculos"):** novo helper `revenueOrdersIn` (filtra por `event_date` + exclui canceladas) usado em `totalRevenue` e no top parceiros; importei `commissionFromOrder` de [finance.ts](src/lib/finance.ts) para a comissão (proporcional + exclui na/nao_aceita); etiqueta dinâmica via `comparisonLabelForPreset`. **Novo pedido da Maria:** ver as Métricas **por defeito "Desde sempre"** e poder mudar para "Este mês" → novo preset `desde_sempre` (range 1970→2999, igual ao "Todos" das Finanças), agora **default** em [metricas-client.tsx](src/app/(admin)/metricas/metricas-client.tsx); flag `showComparison` (false em desde_sempre → esconde badges de variação que não fazem sentido). **Botão refresh global e discreto:** componente `RefreshButton` em [layout.tsx](src/app/(admin)/layout.tsx) usa `router.refresh()` (re-corre Server Components, busca dados frescos sem recarregar a página/sem flash) com ícone `RotateCw` a girar enquanto pendente; montado na sidebar desktop (junto ao tema/som, e também no estado recolhido) e no header mobile (junto à pesquisa). O botão "Actualizar" que já existia na página de Métricas ficou intocado. **Alinhamento das Finanças (pedido da Maria a seguir):** a aba **Faturação** ([financas-client.tsx](src/app/(admin)/financas/financas-client.tsx)) ainda contava encomendas **canceladas** na receita/COGS/comissões (só o "Painel" as excluía). Adicionei o guard `status === "cancelado" → 0` dentro de `revenueFromOrder` e `cogsFromOrder` (cobre mês/ano/mês-anterior/gráfico automaticamente) e o filtro `status !== "cancelado"` nas somas `commissionMonth`/`commissionYear`. Agora Painel + Faturação + Métricas tratam canceladas da mesma maneira. **Sem migração.** `tsc --noEmit` limpo; lint dos ficheiros editados sem erros novos (o erro `setSoundOn` em layout.tsx é **pré-existente**, confirmado por `git stash`). **Maria: push para Vercel** + smoke (não consegui correr o smoke autenticado): abrir Métricas → default "Desde sempre", sem badges de % nos cartões; mudar para "Este mês" → aparecem comparações com a etiqueta certa; Top parceiros com comissões mais baixas/realistas; clicar no novo ícone ↻ (sidebar/topo) actualiza os dados. **Próximo passo guardado (NÃO feito):** expandir a cadência de comunicação (sessão 104).

## Fase anterior: FASE 6 (parte 56) — Sessão 112: **Pill "40% pedidos" na tabela de Preservação** (2026-06-09). A Maria pediu uma pill na lista análoga à "Contactada": serve de aviso de que os 40% já foram **pedidos** ao cliente mas **ainda não foram pagos** (pagamento continua em 30%/por pagar); desaparece assim que paga (70%/100%). Na fase de preservação é suposto toda a gente ter os 40% pagos, daí o sinal. **Onde:** só na **tabela** [preservacao-client.tsx](src/app/(admin)/preservacao/preservacao-client.tsx) (ao lado do nome, a seguir à pill "Contactada", ~linha 578) — o **workbench já tem** o equivalente interactivo (CheckRow "40% pedidos?" no cabeçalho, da sessão 96, que liga a `payment_40_requested`). Mesmo padrão da Contactada: pill na lista ↔ checkbox no workbench. A vista de Cards não mostra Contactada → também não leva esta (consistente). **Condição:** `order.payment_40_requested && currentPayment !== "70_pago" && currentPayment !== "100_pago"`. Pill âmbar com ícone `Euro` (lucide, novo import) + tooltip "Os 40% foram pedidos ao cliente mas ainda não foram pagos". **Sem migração** (`payment_40_requested` existe desde a mig 018). Preflight `tsc + next build` limpos. **Maria: só push para Vercel** + smoke: encomenda em "Flores na prensa" com 30% pago e "40% pedidos?" ticado no workbench → aparece pill âmbar "40% pedidos" na tabela; mudar pagamento para 70% → pill desaparece. **Próximo passo guardado (NÃO feito):** expandir a cadência de comunicação (sessão 104).
