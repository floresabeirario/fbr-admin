...
## Fase actual: FASE 6 (parte 51) — Sessão 107: **Tirar alerta "Parada há X dias" + nova vista "Comissões" por saldar nas Parcerias** (2026-06-04). Dois pedidos da Maria a partir do cartão de Alertas do Dashboard. **(1) Removido o alerta "Parada há X dias"** — era a secção 2 de `getDashboardAlerts` em [src/lib/dashboard.ts](src/lib/dashboard.ts) (encomendas ≥14 dias sem update). Maria: "não sei o que é isto e não preciso". Mesmo conceito que já tínhamos tirado da tabela+workbench na sessão 95 (queixa G); este era o último sítio onde sobrava. Removido o bloco + a const `STUCK_DAYS`; renumeradas as secções (pré-reservas sem contacto → 2, vales a expirar → 3). **(2) Nova vista "Comissões" nas Parcerias** — Maria quer ver que comissões de parceiros ainda não foram pagas ("às vezes não respondem/esquecem-se; quero ter as dívidas todas saldadas"). As comissões vivem em `orders` E `vouchers` (`partner_commission` + `partner_commission_status`). **Helper** [src/lib/commissions.ts](src/lib/commissions.ts) (puro): tipo `CommissionItem`, `COMMISSION_PENDING_STATUSES` (`parceiro_informado`, `a_aguardar_resposta`, `a_aguardar` — exclui `na`/`paga`/`nao_aceita`), `isCommissionDueNow` (devida agora = informado + a aguardar resposta) vs `isCommissionNotYetDue` (`a_aguardar` = "Encomenda não paga na totalidade" → ainda não devida), `groupCommissionsByPartner` (parceiros por total desc, items por mais antigo), `sumCommissions`. **page.tsx** [parcerias/page.tsx](src/app/(admin)/parcerias/page.tsx): +2 queries (`orders`/`vouchers` com `.in("partner_commission_status", pending)`) → consolida em `CommissionItem[]`, passa por `parcerias-tabs.tsx` → `ParceriasClient`. **Vista** [parcerias/commissions-view.tsx](src/app/(admin)/parcerias/commissions-view.tsx) (client): novo `viewMode "comissoes"` (3º botão "€ Comissões" com badge âmbar de contagem, ao lado de Tabela/Mapa); banner "Total por pagar agora" (€), grupos por parceiro (devidas) + secção separada "Ainda não devidas (cliente por pagar)", cada linha com cliente/vale + badge de estado + "há X dias" + [Abrir] + [Marcar paga]. **Action** [parcerias/actions.ts](src/app/(admin)/parcerias/actions.ts): `markCommissionPaidAction(kind, rowId)` → UPDATE `partner_commission_status='paga'` em orders/vouchers; **requireAdmin** (acção financeira — Ana vê a lista mas não marca pagas, ao contrário do resto das Parcerias que é `requireUser`). **Sem migração** (campos já existem desde a mig 018). Preflight `tsc + next build` limpos. **Maria: push para Vercel** + smoke: Dashboard já não mostra "Parada há X dias"; Parcerias → botão "Comissões" → lista agrupada por parceiro com total, "Marcar paga" tira a comissão da lista. **Próximo passo guardado (NÃO feito):** expandir a cadência de comunicação (sessão 104).

## Fase anterior: FASE 6 (parte 50) — Sessão 106: **Gmail + WhatsApp no workbench das Parcerias E das Figuras Públicas** (2026-06-04). A Maria pediu para ter também o histórico de WhatsApp e Gmail a aparecer nas Parcerias (e depois também nas Figuras Públicas). Os dois painéis já eram totalmente genéricos (só dependem de email/telefone, não da encomenda) → reaproveitados por import, sem duplicar código. **(1) Parcerias** [parcerias/[id]/workbench-client.tsx](src/app/(admin)/parcerias/[id]/workbench-client.tsx) e **(2) Figuras Públicas** [parcerias/figura/[id]/workbench-client.tsx](src/app/(admin)/parcerias/figura/[id]/workbench-client.tsx): mesmo componente novo `CommunicationsCard` (accent azul) com tabs Email/WhatsApp, importando `GmailPanel` e `WhatsappLivePanel` de `@/app/(admin)/preservacao/[id]/_components/`. Email usa `partner.email`/`figure.email`; WhatsApp usa `phones[]` — como pode haver vários telefones, há um **dropdown** (só aparece com >1 telefone) para escolher qual conversa mostrar (default = primeiro). **Localização (decisão de fluxo):** card no topo da coluna esquerda, junto ao "Histórico de interações" (registo manual) e Acções → a esquerda passa a ser o "centro de comunicação"; nas Parcerias removeu-se o `lg:sticky` da aside porque a coluna ficou comprida. **Sem migração** (comunicações vivem no Gmail/WhatsApp, não na BD). Preflight `tsc + next build` limpos (corrigido: `onValueChange` da Select devolve `string | null` → `(v) => setSelectedPhone(v ?? "")`). **Feito push para o master** (a Maria pediu). **Maria: smoke** — abrir um parceiro/figura com email/telefone → coluna esquerda tem card "Comunicações" com tab Email (lista threads) e WhatsApp (conversa; dropdown se tiver vários números). **Próximo passo guardado (NÃO feito):** expandir a cadência de comunicação (sessão 104) — +entradas em `COMMS_CADENCE` para sinal 30%, agradecer reserva, pedir 40%, pedir 30% final, avisar "a caminho".

## Fase anterior: FASE 6 (parte 49) — Sessão 105: **Gmail ligado a sério — painel de emails (só leitura) no workbench** (2026-06-04). A tab "Email" do workbench era só um placeholder ("Vai puxar… via Gmail API"). Agora puxa de verdade os emails trocados com cada cliente, na conta `info@floresabeirario.pt`, via o alicerce OAuth já existente (scope `gmail.readonly` já estava em `GOOGLE_SCOPES`). Filosofia **só leitura**, igual ao WhatsApp ([[feedback-nada-de-envio-automatico]]) — o envio continua à mão no Gmail. **(1) Lib** [src/lib/google/gmail.ts](src/lib/google/gmail.ts) (server-only): `fetchThreadsWithContact(email)` reusa `getAuthenticatedClient()`, lista threads com `q: from:X OR to:X` (máx 15), puxa cada thread em `format:full`, descodifica corpo base64url (text/plain → fallback text/html sem tags), marca direcção `sent`/`received` comparando o From com `google_email`, ordena da mais recente para a mais antiga. Devolve estados discriminados `ok | not_connected | missing_scope`. `gmailScopeGranted()` verifica se a ligação guardada inclui o scope. **(2) Rota** [src/app/api/google/emails/route.ts](src/app/api/google/emails/route.ts) admin-only (`getCurrentRole`), `GET ?email=` → JSON (padrão do `/api/whatsapp/suggest`). **(3) Painel** [preservacao/[id]/_components/gmail-panel.tsx](src/app/(admin)/preservacao/[id]/_components/gmail-panel.tsx): threads colapsáveis, bolhas estilo email (FBR azul à direita / cliente à esquerda), `linkify` no corpo, botão actualizar, link "Abrir no Gmail", e empty/erro/`not_connected`/`missing_scope` (estes 2 últimos mandam para /settings/google). Reusado em **Preservação** (`local.email`) e **Vale-Presente** (`data.sender_email`, import relativo — mesmo padrão do `WhatsappLivePanel`). **(4) Ecossistema**: card Gmail passou de "Pendente" a "Activo". **Sem migração** (emails vivem no Gmail, não na BD). Preflight `tsc + next build` limpos (rota `/api/google/emails` no output; único warning é a fonte Google Sans = rede). **Maria: passos manuais:** (1) **push para Vercel**; (2) **garantir o scope do Gmail** — se a tab "Email" mostrar "Falta autorizar o Gmail", ir a **/settings/google → desconectar → Conectar Google** e aceitar o acesso ao email (a ligação antiga pode não ter o `gmail.readonly` concedido); (3) smoke: abrir uma encomenda com email de cliente → tab "Email" lista as conversas; expandir uma → vê as mensagens + "Abrir no Gmail"; repetir num vale com `sender_email`. **Próximo passo guardado (NÃO feito):** expandir a cadência de comunicação (sessão 104) — +entradas em `COMMS_CADENCE` para sinal 30%, agradecer reserva, pedir 40%, pedir 30% final, avisar "a caminho".

## Fase anterior: FASE 6 (parte 48) — Sessão 104: **Motor de cadência de comunicação + 1º momento "pedir opinião" (mig 072)** (2026-06-03). A Maria quer ser lembrada de enviar as mensagens certas em cada momento-chave de uma encomenda — tudo enviado **à mão por WhatsApp, NADA automático** — para não esquecer nada; os Afazeres do Dashboard tornam-se a "lista de mensagens por enviar" que ela vai dando check. Pedido concreto: 2 dias após a encomenda entrar em **"Quadro recebido"**, lembrar de pedir opinião ao cliente, com a mensagem (PT/EN) pronta a copiar. **Decisão de design (brainstorm):** construir um **motor genérico de cadência** mas ligar agora SÓ o momento "pedir opinião"; restantes momentos guardados como próximo passo. **(1) Motor** [src/lib/comms-cadence.ts](src/lib/comms-cadence.ts) (puro, sem IO): tipo `CommsMoment` (trigger por estado e/ou pagamento, templateSlug, taskTitle, category, priority, dueOffsetDays, assignees) + `COMMS_CADENCE` (1 entrada: `pedir_opiniao`, trigger `quadro_recebido`, +2 dias, ambos os admins) + `detectTriggeredMoments()` (dispara quando ENTRA no estado-alvo e ainda não está em `alreadyDone` → idempotente) + `dueDateFromOffset()`. **(2) Sem cron** — a tarefa é criada na transição com `due_date`=+2 dias, reusando o padrão das tarefas de fatura em [actions.ts](src/app/(admin)/preservacao/actions.ts) (`updateOrderAction`): `prev` ganha `comms_moments_done` no select; após o UPDATE cria 1 tarefa por momento e marca `orders.comms_moments_done` (silencioso em falha). **(3) Mensagem pronta** — 2 templates PT+EN (`pedir_opiniao_quadro_pt/en`, categoria `pos_venda`, sugeridos em `quadro_recebido`) com nova variável **`{link_avaliacao}`**. **(4) Link editável** — nova definição `review_link` em `system_settings` (variável `{link_avaliacao}` em [templates.ts](src/lib/templates.ts)), com cartão próprio "Link de opinião" em Sistema → Templates ([templates-client.tsx](src/app/(admin)/comunicacoes/templates/templates-client.tsx)); seed = link Google Maps da FBR (trocável por link curto/página futura). **Verdade técnica fixada:** o Google não permite publicar reviews a partir de fora → uma **página própria de opiniões** (form no site, funciona sem conta Google) fica como tarefa futura no `fbr-website`. **Mig 072** ([072_cadencia_comunicacao_opiniao.sql](supabase/migrations/072_cadencia_comunicacao_opiniao.sql)): `ALTER orders ADD comms_moments_done text[]` + 2 templates + `system_settings.review_link` — só ALTER/INSERT em tabelas existentes, sem grants novos ([[project-supabase-public-grants-2026]] não se aplica). **Tipos:** `Order.comms_moments_done`, `SystemSettingKey` ganha `review_link`; `ADMIN_EMAILS_LIST` exportado de [roles.ts](src/lib/auth/roles.ts). Preflight `tsc + next build` limpos. **Próximo passo guardado (NÃO feito):** expandir a cadência (+1 entrada em `COMMS_CADENCE` + templates por cada): pré-reserva→pedir sinal 30%; 30% pago→agradecer+confirmar reserva; flores recebidas→pedir 40% (ligar aos avisos "40% pedidos?" da sessão 96); quadro pronto→pedir 30% final (idem "30% pedidos?"); quadro enviado→avisar a caminho. **Maria: passos manuais:** (1) correr **[mig 072](supabase/migrations/072_cadencia_comunicacao_opiniao.sql)** no Supabase SQL Editor ([[feedback-migracoes-supabase-aplicadas]]); (2) push para Vercel; (3) smoke: Sistema → Templates tem cartão "Link de opinião"; abrir encomenda em "Quadro recebido" → picker sugere "Pós-venda — pedir opinião" (PT/EN) com nome+link; mudar uma encomenda para "Quadro recebido" → tarefa "Pedir opinião sobre o quadro — {cliente}" nos Afazeres (2 admins, prazo +2 dias), e repetir a transição NÃO duplica.
