...
## Fase actual: FASE 6 (parte 41) — Sessão 97: **WhatsApp em tempo real (Coexistence + Cloud API)** (2026-05-30, EM CURSO). Integração viva substituindo o registo manual da mig 042. **Decisões fixadas em conversa:** (1) **uma conversa por número de telefone**, independente de quantas encomendas a pessoa tem; (2) Caixa de Entrada nova no menu para conversas sem encomenda (leads); (3) **AI só corre quando Maria carrega no botão "Sugerir resposta"** + caixa de texto opcional para dar instrução (ex: "diz que sim, conseguimos fazer o que ela quer") — templates da mig 041 continuam a resolver 80% dos casos sem gastar tokens; (4) **multimédia puxada em background** (webhook responde 200 em <10s para evitar retransmissões da Meta) e guardada na Drive da cliente; (5) tab WhatsApp manual será **substituída** com export prévio do `orders.whatsapp_log` para JSON na Drive de cada encomenda. **Modelo Claude:** `claude-sonnet-4-6` (constante em `src/lib/claude.ts` fácil de trocar para `claude-haiku-4-5-20251001`). **Estado actual:** migração 061 escrita ([supabase/migrations/061_whatsapp_live.sql](supabase/migrations/061_whatsapp_live.sql)) — 2 tabelas (`whatsapp_conversations`, `whatsapp_messages`), trigger de sumário, RPC `mark_whatsapp_conversation_read`, RLS (admins all + Ana read), Realtime publicado. **Próximo passo:** Maria corre mig 061; eu construo endpoint do webhook (handshake GET + POST com HMAC `META_APP_SECRET`) e fazemos deploy para o URL público ir para o Dualhook.
