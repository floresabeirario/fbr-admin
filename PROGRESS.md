...
## Fase actual: FASE 6 (parte 48) — Sessão 104: **Motor de cadência de comunicação + 1º momento "pedir opinião" (mig 072)** (2026-06-03). A Maria quer ser lembrada de enviar as mensagens certas em cada momento-chave de uma encomenda — tudo enviado **à mão por WhatsApp, NADA automático** — para não esquecer nada; os Afazeres do Dashboard tornam-se a "lista de mensagens por enviar" que ela vai dando check. Pedido concreto: 2 dias após a encomenda entrar em **"Quadro recebido"**, lembrar de pedir opinião ao cliente, com a mensagem (PT/EN) pronta a copiar. **Decisão de design (brainstorm):** construir um **motor genérico de cadência** mas ligar agora SÓ o momento "pedir opinião"; restantes momentos guardados como próximo passo. **(1) Motor** [src/lib/comms-cadence.ts](src/lib/comms-cadence.ts) (puro, sem IO): tipo `CommsMoment` (trigger por estado e/ou pagamento, templateSlug, taskTitle, category, priority, dueOffsetDays, assignees) + `COMMS_CADENCE` (1 entrada: `pedir_opiniao`, trigger `quadro_recebido`, +2 dias, ambos os admins) + `detectTriggeredMoments()` (dispara quando ENTRA no estado-alvo e ainda não está em `alreadyDone` → idempotente) + `dueDateFromOffset()`. **(2) Sem cron** — a tarefa é criada na transição com `due_date`=+2 dias, reusando o padrão das tarefas de fatura em [actions.ts](src/app/(admin)/preservacao/actions.ts) (`updateOrderAction`): `prev` ganha `comms_moments_done` no select; após o UPDATE cria 1 tarefa por momento e marca `orders.comms_moments_done` (silencioso em falha). **(3) Mensagem pronta** — 2 templates PT+EN (`pedir_opiniao_quadro_pt/en`, categoria `pos_venda`, sugeridos em `quadro_recebido`) com nova variável **`{link_avaliacao}`**. **(4) Link editável** — nova definição `review_link` em `system_settings` (variável `{link_avaliacao}` em [templates.ts](src/lib/templates.ts)), com cartão próprio "Link de opinião" em Sistema → Templates ([templates-client.tsx](src/app/(admin)/comunicacoes/templates/templates-client.tsx)); seed = link Google Maps da FBR (trocável por link curto/página futura). **Verdade técnica fixada:** o Google não permite publicar reviews a partir de fora → uma **página própria de opiniões** (form no site, funciona sem conta Google) fica como tarefa futura no `fbr-website`. **Mig 072** ([072_cadencia_comunicacao_opiniao.sql](supabase/migrations/072_cadencia_comunicacao_opiniao.sql)): `ALTER orders ADD comms_moments_done text[]` + 2 templates + `system_settings.review_link` — só ALTER/INSERT em tabelas existentes, sem grants novos ([[project-supabase-public-grants-2026]] não se aplica). **Tipos:** `Order.comms_moments_done`, `SystemSettingKey` ganha `review_link`; `ADMIN_EMAILS_LIST` exportado de [roles.ts](src/lib/auth/roles.ts). Preflight `tsc + next build` limpos. **Próximo passo guardado (NÃO feito):** expandir a cadência (+1 entrada em `COMMS_CADENCE` + templates por cada): pré-reserva→pedir sinal 30%; 30% pago→agradecer+confirmar reserva; flores recebidas→pedir 40% (ligar aos avisos "40% pedidos?" da sessão 96); quadro pronto→pedir 30% final (idem "30% pedidos?"); quadro enviado→avisar a caminho. **Maria: passos manuais:** (1) correr **[mig 072](supabase/migrations/072_cadencia_comunicacao_opiniao.sql)** no Supabase SQL Editor ([[feedback-migracoes-supabase-aplicadas]]); (2) push para Vercel; (3) smoke: Sistema → Templates tem cartão "Link de opinião"; abrir encomenda em "Quadro recebido" → picker sugere "Pós-venda — pedir opinião" (PT/EN) com nome+link; mudar uma encomenda para "Quadro recebido" → tarefa "Pedir opinião sobre o quadro — {cliente}" nos Afazeres (2 admins, prazo +2 dias), e repetir a transição NÃO duplica.
