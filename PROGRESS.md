...
## Fase actual: FASE 6 (parte 46) — Sessão 102: **Ecossistema actualizado para reflectir a realidade (WhatsApp live + Dualhook + Claude)** (2026-06-02). A página [ecossistema/page.tsx](src/app/(admin)/ecossistema/page.tsx) estava desactualizada: WhatsApp aparecia como "Pendente — registo manual, WhatsApp não tem API pública" e Claude como "Por integrar", e o **Dualhook nem existia**. Tudo isto mudou nas sessões 97-99. Correcções no array `INTEGRATIONS`: (1) **Dualhook** adicionado (card novo, `Webhook` icon, Activo) — relay que reencaminha os webhooks do WhatsApp/Meta Cloud API, multi-tenant (não expõe App Secret → validamos por token no path). (2) **WhatsApp (Meta Cloud API)** → Activo: caixa de entrada `/whatsapp` em tempo real, ecos das respostas, multimédia na Drive, estados entrega/leitura, tab live no workbench; nota deixa claro "recepção via Dualhook, envio não implementado (0€ Meta)". (3) **Anthropic Claude** → Activo: "Cérebro do Claudio", Sonnet 4.6 com prompt caching, só corre ao carregar no botão. (4) **Gmail** continua Pendente (real — scope OAuth pedido em [oauth.ts](src/lib/google/oauth.ts) mas sem painel no workbench); nota afinada. Import de `Webhook` do lucide (confirmado existir nesta versão, ao contrário do Instagram). `tsc --noEmit` limpo; `next build` só falhou a buscar a fonte Google Sans ao Google Fonts (rede do ambiente, não relacionado). **Maria: push para Vercel** + abrir `/ecossistema` → secção Integrações mostra Dualhook + WhatsApp + Claude a verde "Activo". (Follow-up: card do Dualhook ganhou `url` clicável para o dashboard de conexões `dualhook.com/dashboard/connections/conn_PXyQaTPUlQrz`; `IntegrationCard` passou a renderizar o nome como `<a>` quando há `url`.)

## Fase anterior: FASE 6 (parte 45) — Sessão 101: **Figuras Públicas — par/cônjuge (casais) + inverter toggle + simplificar tipos (mig 071)** (2026-06-02). Pós-uso da sessão 100. **(1) Toggle invertido:** em [parcerias-tabs.tsx](src/app/(admin)/parcerias/parcerias-tabs.tsx) os dois botões trocaram de ordem — **Figuras Públicas** passa a ser o da esquerda e o modo default (`useState<Mode>("figuras")`); Parcerias fica à direita. (2) **Casais (decisão Maria):** "quem casa é um casal" — às vezes AMBAS as pessoas são figuras públicas, às vezes só uma é e a outra é o cônjuge. Modelado com UM só registo + campos opcionais do par: `partner_name` (sempre que há par), `partner_instagram` + `partner_followers` (só se o par também for figura). **Mig 071** ([071_public_figures_partner.sql](supabase/migrations/071_public_figures_partner.sql)): `ADD COLUMN partner_name/partner_instagram/partner_followers` (ALTER à tabela existente → sem grants novos, [[project-supabase-public-grants-2026]] só afecta tabelas novas). Helper `figureDisplayName()` em [public-figure.ts](src/types/public-figure.ts) mostra "Sofia & João" no título do workbench + linha da tabela; @ do par a seguir ao @ principal na listagem; pesquisa inclui partner_name+partner_instagram. Form "Nova figura" e workbench: campo "Par / cônjuge" na Identificação + bloco tracejado de redes do par (só aparece quando há `partner_name`). (3) **Tipos simplificados:** "Celebridade" fundida em "Figura pública" (a distinção era só semântica, nenhum comportamento ligado a `figure_type`) → `FigureType = "influencer" | "figura_publica"`. Mig 071 faz `UPDATE figure_type='figura_publica' WHERE='celebridade'` + recoloca o CHECK sem 'celebridade'. Preflight `tsc + next build` limpos. **Maria: passos manuais:** (1) **correr [mig 071](supabase/migrations/071_public_figures_partner.sql)** no Supabase SQL Editor ([[feedback-migracoes-supabase-aplicadas]]); (2) push para Vercel; (3) smoke: Parcerias abre em "Figuras Públicas" → Nova figura com par → workbench mostra "Nome & Par" + redes do par.

## Fase anterior: FASE 6 (parte 44) — Sessão 100: **Figuras Públicas — gestão de contactos a influencers/noivas (nova grande secção dentro de Parcerias, mig 070)** (2026-06-01). Maria quer começar a contactar influencers e figuras públicas (sobretudo noivas) para oferecer a preservação das flores como cortesia em troca de exposição (*seeding*). Em vez de nova aba na sidebar, a aba **Parcerias** passa a ter duas grandes partes via toggle no topo: **Parcerias** (recomendadores, intocado) e **Figuras Públicas** (novo). **Arquitectura:** wrapper [parcerias-tabs.tsx](src/app/(admin)/parcerias/parcerias-tabs.tsx) com toggle + `useState` de modo; renderiza `ParceriasClient` (existente, sem mexer) ou `FigurasClient` (novo). Tabela `public_figures` separada da `partners` (funil/campos próprios), modelada na mig 013. **Mig 070** ([070_public_figures.sql](supabase/migrations/070_public_figures.sql)): tabela com funil próprio (por_contactar→contactada→em_conversa→aceitou→em_producao→concluida→recusou/sem_resposta), prioridade, instagram/tiktok/followers, tags+fit_note, event_type+event_date, offer_type+frame_size+estimated_cost, deliverables JSONB (contrapartida checklist), story_screenshots[], brief (sent/mention/hashtag), contacto+agência, order_id→orders (cortesia, is_courtesy default true → NÃO conta faturação), interactions+actions JSONB (mesma forma das partners). RLS para os 3 users (Ana edita), audit log, grants authenticated+service_role (defensivo, [[feedback-supabase-rls-pitfalls]]). **Tipos:** [src/types/public-figure.ts](src/types/public-figure.ts) (reaproveita PartnerPhone/Interaction/Action). **Actions:** [figuras-actions.ts](src/app/(admin)/parcerias/figuras-actions.ts) (CRUD + interações + acções + deliverables, done_by carimbado server-side). **Helpers:** [src/lib/supabase/public-figures.ts](src/lib/supabase/public-figures.ts) (group/search/figureStats com taxa resposta+aceitação+alcance, daysUntilEvent). **Templates de abordagem:** [src/lib/outreach-templates.ts](src/lib/outreach-templates.ts) (PT+EN: 1º contacto, follow-up, brief, agradecimento; auto-contidos, não usam message_templates). **Listagem:** [figuras-client.tsx](src/app/(admin)/parcerias/figuras-client.tsx) — mini-painel de métricas (4 KPIs), tabela agrupada por estado (prioridade primeiro), alerta follow-up (contactada 7+ dias sem update), alerta de evento próximo (aceitou/em_producao ≤14 dias). **Workbench:** [figura/[id]/workbench-client.tsx](src/app/(admin)/parcerias/figura/[id]/workbench-client.tsx) 3 colunas — esquerda (interações+contrapartida+acções), meio (identificação/redes/evento/oferta com custo sugerido do catálogo/brief/contacto/notas), direita (templates abordagem com copy, screenshots Drive, encomenda ligada, metadata). **Custo estimado:** derivado em [figura/[id]/page.tsx](src/app/(admin)/parcerias/figura/[id]/page.tsx) via `computeProductionCost` (caso-base baixa+transparente) por tamanho → sugestão "usar X€". **Decisões Maria (perguntas respondidas):** secção dentro de Parcerias (não nova aba); nome "Figuras Públicas"; foco noivas mas permite batizado/outro; Ana edita; extras escolhidos = métricas + custo estimado + templates + tags/fit; **código de referência/promo REJEITADO para já → Ideias Futuras**. **WorkbenchNavKey** ganhou `"public_figures"`. **Icon Instagram não existe no lucide-react novo → usei AtSign.** Preflight `tsc + next build` limpos; rota `/parcerias/figura/[id]` no output. **Maria: passos manuais:** (1) **correr [mig 070](supabase/migrations/070_public_figures.sql)** no Supabase SQL Editor; (2) **push para Vercel**; (3) smoke: abrir Parcerias → toggle "Figuras Públicas" → "Nova figura" → abrir workbench, testar interações/contrapartida/templates/custo sugerido. **Ideias Futuras (não feitas, guardadas):** código de referência único por figura (medir conversões via form público, precisa fbr-website); mapa de Portugal dos casamentos; embaixadora recorrente; re-contacto no aniversário de casamento. **Próximos passos possíveis (sessão 101):** export `orders.whatsapp_log` JSONB para Drive; PWA push notifications; ou extras das Ideias Futuras das Figuras Públicas.

## Fase anterior: FASE 6 (parte 43) — Sessão 99: **WhatsApp polish round 1+2 + mig 065 (service_role grants em google_integration e system_settings)** (2026-05-31). Após sessão 97 que entregou backend+UI+IA do WhatsApp, fizemos cleanup e polish. **Bug do dia:** media fetch falhava com `42501 permission denied for table google_integration` — service_role tem grants em whatsapp_* (mig 062) mas não em `google_integration` nem em `system_settings`. Mig 065 corrige ambos. Mesmo bug latente afectava cron healthcheck (que faz upsert em system_settings). Memória `feedback_supabase_rls_pitfalls` actualizada com regra defensiva: **dar grant a service_role em qualquer tabela nova**. **Round 1 polish (commit 2ca91d7):** linkify URLs em texto das bolhas; indicador `⚠ não consegui guardar` quando media_pending=false + sem media_url_drive + media_id presente; apagado dead code (`src/components/whatsapp-log.tsx`, `src/app/(admin)/preservacao/whatsapp-actions.ts`, `src/lib/whatsapp-import.ts` — 1k linhas a menos; tab manual WhatsApp foi substituída por live na sessão 97). **Round 2 features (commit a721095):** chips de filtro na inbox (Todas/Não lidas/Com encomenda/Sem encomenda); botão "marcar não-lida" (poe unread_count=1 + fecha selecção); botão retry na bolha de media falhada via `/api/whatsapp/retry-media` (se URL Meta já expirou falha graciosamente). **Maria: passos manuais:** correr mig 065 (`GRANT SELECT, INSERT, UPDATE, DELETE ON google_integration, system_settings TO service_role;`); confirmar /healthchecks tem status actualizado nas últimas 24h (provavelmente estava em erro silencioso antes). **Próximos passos (sessão 100):** export `orders.whatsapp_log` JSONB antigo para Drive + DROP column; inline thumbnails para fotos; reply context (mostrar mensagem original quando cliente faz reply); push notifications PWA.

## Fase anterior: FASE 6 (parte 42) — Sessão 98: **Mobile fixes — tabela Cliente colapsado + workbench header amontoado + Grid2 a 2-cols indevidamente** (2026-05-31). Maria reportou 3 problemas no telemóvel com screenshots: (a) **Tabela de `/preservacao`** — header "DATA/CLIENTE/EVENTO" sobreposto e valores nome/data a colidirem. Causa: na sessão 96 o `minWidth: 830` era exactamente a soma das colunas fixas (40+110+140+200+110+150+80) → coluna "Cliente" (sem `width`, elástica) colapsava a 0px com `tableLayout: fixed`. Fix em [preservacao-client.tsx:769-774](src/app/(admin)/preservacao/preservacao-client.tsx#L769-L774): `minWidth: 830 + 200 + extras` para garantir 200px mínimos à Cliente. (b) **Header do workbench em mobile** — "Sonia Inacio" truncado para "Soni…", "Contactada" a tapar o ID `#LSCD…`. Causa: sessão 96 trocou `basis-full` por `flex-1` para o bloco nome+ID, fazendo-os lutar por espaço com CheckRows + Nota na mesma linha. Fix em [workbench-client.tsx:672-675](src/app/(admin)/preservacao/[id]/workbench-client.tsx#L672-L675): `basis-full sm:basis-auto sm:flex-1` — mobile dá linha própria ao nome, sm+ partilha como antes (desktop intocado per [[feedback-desktop-prioridade]]). (c) **Secção "Flores, quadro e extras"** com labels sobrepostos ("Tamanho da moldura"/"Fundo do quadro" + "Moldura pirâmide"/"Tipo de moldura (interno)" + selects e hints em pares lado-a-lado). Causa não totalmente diagnosticada — em viewport phone normal o `lg:grid-cols-2` (1024px) NÃO deveria activar, mas screenshot mostra 2 cols. Fix defensivo em [_components/layout.tsx:76-82](src/app/(admin)/preservacao/[id]/_components/layout.tsx#L76-L82): `lg:grid-cols-2` → `xl:grid-cols-2` (Grid2 só vai a 2 cols a partir de 1280px). Em qualquer phone/tablet fica garantidamente 1-col. **Sem migrações.** Preflight `tsc + next build` limpos. **Maria: push para Vercel + smoke browser mobile**: tabela `/preservacao` com colunas alinhadas e Cliente visível, workbench encomenda com nome em linha própria + Contactada/Nota a descer, secção Flores empilhada vertical com selects largos.

## Fase anterior: FASE 6 (parte 41) — Sessão 97: **WhatsApp end-to-end (incluindo media fetch + statuses)** (2026-05-30/31). MARATHON. Todos os itens da TODO da sessão fechados. Próximas sessões: export do JSONB antigo + outras features. **Resumo do que ficou pronto e em produção:** (1) Backend WhatsApp completo (mig 061+062+063, webhook com path token, parser idempotente, Realtime). (2) Página `/whatsapp` (inbox + viewer com bolhas + Sugerir resposta inline + busca + arquivar + notas) com sidebar entry e bolinha emerald. (3) Rota `/api/whatsapp/suggest` com Claude Sonnet 4.6 (constante `CLAUDE_MODEL` em `src/lib/claude.ts`) + prompt caching em 2 blocos (persona + 29 templates). (4) **Cérebro do Claudio** em `/comunicacoes/claudio` (era `/settings/ai`): persona + factos editáveis, lidos pela rota suggest em vez de hardcoded; PERSONA_FALLBACK no código defensivo. (5) **Reorg para `/comunicacoes` hub** com 4 sub-páginas (WhatsApp, Chat interno, Templates, Cérebro do Claudio); sidebar reorganizada — Comunicações como parent dos 4; removido "Mensagens" do topbar Sistema; redirects de `/settings/templates` e `/settings/ai` para os novos URLs. (6) **Workbench WhatsApp tab passa a ser LIVE** (`_components/wa-live-panel.tsx`): procura conversa por last 9 digits de `orders.phone`, Realtime + empty states + Sugerir embutido + link "Caixa de entrada"; manual `WhatsAppLog` em src/components fica dead code (exportar JSONB para Drive na próxima). **Decisões fixadas:** (a) uma conversa por número de telefone independente de encomendas; (b) AI só corre quando carregas botão; (c) multimédia puxada em background; (d) tab manual substituída agora (export pendente). **Modelo:** `claude-sonnet-4-6`; constante fácil de trocar para `claude-haiku-4-5-20251001`. **Descobertas relevantes:** Dualhook NÃO expõe App Secret (multi-tenant); Meta envia `x-hub-signature-256` mas secret é deles → usamos path token; WhatsApp **Windows Desktop NÃO gera echoes** (limitação Meta — só Mobile + Web); statuses (delivery/read) chegam mas são ignorados. **Custo:** receber = grátis; enviar (não implementado) = pago; Claude com cache ~€2-5/mês. Endpoint de envio nunca foi/será construído nesta fase para garantir 0€ Meta. **Env vars manuais que foram adicionadas no Vercel pela Maria:** `WHATSAPP_VERIFY_TOKEN`, `ANTHROPIC_API_KEY` (e temporariamente `WHATSAPP_DEBUG_ACCEPT_UNSIGNED` que ela removeu após path token). **Migrações aplicadas no Supabase:** 061 + 062 + 063. **Migrações por aplicar:** 064 (statuses — Maria corre na sessão actual). **Migrações desta sessão (parte 2):** mig 064 + lib/whatsapp/media-fetch.ts + após-resposta via `after()` da Next 16 (background até 60s). **Media fetch (parte 2):** quando chega multimédia → webhook responde 200 → background puxa de `graph.facebook.com/v25.0/{media_id}` (5min validade) → upload para `<FBR Root>/WhatsApp media/<phone_e164>/<ts>-<wamid>.<ext>` via `uploadWhatsappMedia` helper → UPDATE com `media_url_drive` (Realtime UPDATE → bolhas mostram "Abrir na Drive ↗"). Necessita `WHATSAPP_ACCESS_TOKEN` no Vercel (Bearer da Meta Cloud API — fornecido pelo Dualhook). **Statuses (parte 2):** webhook parser handles `value.statuses[]` no mesmo evento que `messages`; UPDATE por wamid com `delivery_status` + `delivered_at`/`read_at`; UI bolhas mostram ticks só para sent_echo: 📱 (sem status) / ✓✓ cinza (delivered) / ✓✓ azul (read) / ⚠ vermelho (failed). **Próximos passos (sessão 98+):** (i) export `orders.whatsapp_log` JSONB antigo para Drive + drop column; (ii) eventualmente PWA push notifications para novas mensagens. **Decisões fixadas em conversa:** (1) **uma conversa por número de telefone**, independente de quantas encomendas a pessoa tem; (2) Caixa de Entrada `/whatsapp` no menu para conversas sem encomenda (leads); (3) **AI só corre quando Maria carrega no botão "Sugerir resposta"** + caixa de texto opcional para dar instrução (ex: "diz que sim, conseguimos fazer o que ela quer") — templates da mig 041 resolvem 80% sem gastar tokens; (4) **multimédia puxada em background** (webhook responde 200 em <10s) — Drive da cliente; (5) tab WhatsApp manual (mig 042) será substituída em sessão futura com export prévio. **Modelo Claude:** `claude-sonnet-4-6` (constante a definir em `src/lib/claude.ts`). **O que foi feito:** (A) Migrações 061 + 062 — 2 tabelas `whatsapp_conversations` + `whatsapp_messages`, trigger de sumário, RPC `mark_whatsapp_conversation_read`, RLS (admins all + Ana read), Realtime publicado, GRANTs para `service_role`. (B) Webhook `/api/whatsapp/webhook/[token]` com handshake GET + POST, validação por **path token** (reusa `WHATSAPP_VERIFY_TOKEN`, timing-safe comparison, 404 se token errado). HMAC opcional se `META_APP_SECRET` vier algum dia. (C) Parser idempotente por `wamid`; auto-cria conversa via `contacts[0].wa_id`; handles received + sent_echo (smb_message_echoes); ignora statuses. (D) Hook `useUnreadWhatsappCount` + entrada na sidebar "WhatsApp" com bolinha emerald. (E) Página `/whatsapp` com lista de conversas + viewer com bolhas, Realtime, busca, arquivar, notas. **Descobertas relevantes:** Dualhook NÃO expõe App Secret (multi-tenant — Meta App é deles); a Meta envia `x-hub-signature-256` mas o secret é deles; mensagens do **WhatsApp Windows Desktop NÃO geram echoes** (Meta limitation — só Mobile + Web); statuses (delivery/read) chegam separadamente e são ignorados em v1. **Migrações aplicadas no Supabase:** 061 + 062. **Estado actual da sessão:** backend funcional, mensagens reais a ser inseridas; UI inbox + viewer prontos. **Próximos passos (sessão 98):** workbench WA tab (substituir manual com export prévio do `orders.whatsapp_log`); rota Claude "Sugerir resposta"; media fetch job (Drive).
