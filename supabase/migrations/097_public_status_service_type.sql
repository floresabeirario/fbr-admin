-- ============================================================
-- 097 — Fase pública sabe o service_type (esconder "Flores na prensa"
--        nas encomendas de "Emoldurar Flores Já Secas")
--
-- PORQUÊ: as flores já secas NÃO passam pela prensa. No site público
-- (status.floresabeirario.pt) a timeline mostrava sempre os 12 passos,
-- incluindo "Flores na prensa" (fase 3), o que confunde quem já enviou
-- flores secas. O fbr-tracking precisa de saber o tipo de serviço para
-- omitir esse passo do roteiro. A fase pública continua a ser a fonte
-- única em runtime (esta RPC), por isso é aqui que se expõe.
--
-- O QUE MUDA:
--   1) GRANT SELECT (service_type) ao anon — a RPC é SECURITY INVOKER,
--      logo corre com os direitos do anon; sem este grant o SELECT do
--      service_type dentro da função falharia.
--   2) CREATE OR REPLACE da RPC get_public_order_status para devolver
--      também 'service_type' (default 'preservacao'). Todo o resto do
--      corpo (mapeamento estado→fase, labels, mensagens) é IDÊNTICO à
--      migração 092 — não muda nada no mapeamento nem nos textos.
--
-- NOTA DE SINCRONIA: esta migração passa a ser a definição VIVA da RPC.
-- O teste public-status-sync.test.ts foi reapontado para este ficheiro
-- (labels/mensagens têm de continuar iguais a src/lib/public-status.ts).
--
-- ROLLOUT: aditiva. Correr no Supabase SQL Editor ANTES do deploy do
-- fbr-tracking que consome o novo campo.
-- ============================================================

-- ── 1. Column-level GRANT: anon passa a poder ler service_type ──
-- (mantém os grants das migs 017/020/032; só acrescenta esta coluna,
--  que não tem PII — é só o discriminador do tipo de serviço)
GRANT SELECT (service_type) ON orders TO anon;

-- ── 2. Redefinição da RPC (só acrescenta service_type ao output) ──
...
comment on function public.get_public_order_status(text) is
  'Fonte única da fase pública para os sites públicos (fbr-tracking). Devolve service_type para o tracking esconder passos que não se aplicam (ex.: "Flores na prensa" nas flores já secas). Espelha src/lib/public-status.ts; guardado por public-status-sync.test.ts. Ver docs/ECOSYSTEM.md.';
