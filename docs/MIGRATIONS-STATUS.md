...
| `113_status_history_and_whatsapp_response` | Tabela nova `order_status_history` (trigger em `orders.status` + backfill do `audit_log`; RLS `is_team_member`, GRANT authenticated/service_role) e função `whatsapp_first_response()` (1.ª mensagem enviada depois de cada pedido, pelo telemóvel). Para as Métricas: dias em cada fase e tempo de resposta. Ordem indiferente (a página trata tabela/função em falta como "sem dados"). Sessão 174. | ✅ 17/09 |
| `112_fix_imported_payment_stamps` | Corrige a mig 111: o backfill leu os INSERTs da importação do Monday como pagamentos e carimbou a data da importação; esta apaga carimbos colados ao `created_at` em encomendas anteriores ao 1.º `consent_at` (voltam à data do evento). Só dados, idempotente. Sessão 174. | ✅ 17/09 |
