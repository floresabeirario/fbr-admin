...
export default async function FinancasPage() {
  ...
  const [competitorsRes, pricingRes, productionCostRes, expensesRes, ordersRes, vouchersRes] = await Promise.all([
    ...
    supabase
      .from("orders")
      .select("id, order_id, created_at, event_date, status, payment_status, budget, frame_delivery_date, frame_size, frame_background, pyramid_frame, frame_internal_type, extra_small_frames, extra_small_frames_qty, production_cost_snapshot")
      .is("deleted_at", null),
    supabase
      .from("vouchers")
  ...
  const orders = (ordersRes.data ?? []) as Pick<Order, "id" | "order_id" | "created_at" | "event_date" | "status" | "payment_status" | "budget" | "frame_delivery_date" | "frame_size" | "frame_background" | "pyramid_frame" | "frame_internal_type" | "extra_small_frames" | "extra_small_frames_qty" | "production_cost_snapshot">[];
